 ...
 * A PFI is a formal quotation sent to a buyer before grain is sourced.
 * Lifecycle: draft → sent → accepted | rejected | expired
 *
 * On accept the linked BuyerOrder advances to draft automatically (backend).
 */
...
const CreatePFIForm: FC<{
  ...
  useEffect(() => {
    ...
    SourcingService.getBuyerOrders({ status: "quotation", page_size: 100 })
      .then(r => setBuyerOrders((r.results || []).map((o: any) => ({ value: o.id, label: `${o.order_number} — ${o.buyer_detail?.business_name || o.buyer_name}` }))))
      .catch(() => {});
  ...
  const form = useFormik({
    ...
    onSubmit: async (values) => {
      setLoading(true);
      try {
        const pfi = await SourcingService.createProformaInvoice({
          buyer_order: values.buyer_order,
          grain_type: values.grain_type,
          quantity_kg: values.quantity_kg as any,
          unit_price: values.unit_price as any,
          required_deposit: values.required_deposit as any,
        ...
        toast.error(
          err?.buyer_order?.[0] || err?.grain_type?.[0] || err?.quantity_kg?.[0] ||
          err?.unit_price?.[0] || err?.non_field_errors?.[0] || "Failed to create PFI"
        );
        ...
        <Grid container spacing={2} sx={{ pt: 1 }}>
          <Grid item xs={12}>
            <FormControl fullWidth error={Boolean(form.touched.buyer_order && form.errors.buyer_order)}>
              ...
              <Select value={form.values.buyer_order} label="Buyer Order *"
                onChange={e => form.setFieldValue("buyer_order", e.target.value)}>
                {buyerOrders.map(o => <MenuItem key={o.value} value={o.value}>{o.label}</MenuItem>)}
              </Select>
          ...
          <Grid item xs={12} md={4}>
            <TextField fullWidth label="Ship Date" value={form.values.ship_date}
              ...
              helperText="e.g. 'Various' or specific date" />
          </Grid>
          <Grid item xs={12} md={4}>
...
const ProformaInvoices: FC = () => {
  ...
  const handleAction = async (pfi: IProformaInvoice, action: "send" | "accept" | "expire") => {
    ...
    try {
      ...
      else if (action === "accept") await SourcingService.acceptProformaInvoice(pfi.id);
      else if (action === "expire") await SourcingService.expireProformaInvoice(pfi.id);
            ...
            <TableBody>
              {pfis.length === 0 ? (
                ...
                <TableRow key={pfi.id} hover>
                  ...
                  <TableCell>
                    <Typography
                      variant="body2" color="primary" sx={{ cursor: "pointer", fontWeight: 600 }}
                  ...
                  <TableCell sx={{ fontSize: 12 }}>{pfi.sent_at ? formatDateToDDMMYYYY(pfi.sent_at) : "—"}</TableCell>
                  <TableCell>
