  ...
  return (
    <ModalDialog title="New Buyer Order" onClose={handleClose} id={modalId} open={true} ActionButtons={() => (<><Button onClick={handleClose} disabled={loading}>Cancel</Button><Button variant="contained" onClick={() => form.handleSubmit()} disabled={loading}>{loading ? <ProgressIndicator color="inherit" size={20} /> : "Create & Open Order"}</Button></>)} maxWidth="md">
      <Grid container spacing={2} sx={{ pt: 1 }}>
        ...
        {/* Document unit — kg or MT. Controls display on all PDFs only; data always in kg. */}
        <Grid item xs={12} md={6}>
          <FormControl fullWidth>
            <InputLabel>Document Unit</InputLabel>
            <Select value={form.values.trade_unit} label="Document Unit" onChange={e => form.setFieldValue("trade_unit", e.target.value)}>
...
const BuyerOrders: FC = () => {
  ...
  const columns = [
    ...
    { Header: "Gross Profit", accessor: "gross_profit", minWidth: 130, Cell: ({ row }: any) => <Span sx={{ fontWeight: 600, color: row.original.gross_profit >= 0 ? "success.main" : "error.main" }}>{formatCurrency(row.original.gross_profit, "UGX")}</Span> },
