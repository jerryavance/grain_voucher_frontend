...
const RecordPaymentForm: FC<{
  ...
  const form = useFormik({
    ...
    validationSchema: Yup.object({
      amount: Yup.number()
        .positive()
        .max(invoice.balance_due, `Max: ${formatCurrency(invoice.balance_due)}`)
        .required("Amount required"),
      method: Yup.string().required(),
      reference_number: Yup.string().required("Reference number required"),
    }),
    onSubmit: async (values, { setSubmitting }) => {
      ...
      try {
        ...
        if ((values as any).__confirmMode) {
          ...
          toast.success("Payment saved as pending");
        }
        callBack?.();
        handleClose();
      } catch (e: any) {
        toast.error(e?.response?.data?.amount?.[0] || "Failed to record payment");
      } finally {
        setLoading(false);
        setSubmitting(false);
      }
  ...
  const submitAs = async (confirmMode: boolean) => {
    await form.setFieldValue("__confirmMode" as any, confirmMode);
    form.handleSubmit();
  };
  ...
  const ActionBtns: FC = () => (
    <>
      ...
      <Button variant="outlined" onClick={() => submitAs(false)} disabled={loading}>
        Save as Pending
      </Button>
      <Button variant="contained" color="success" onClick={() => submitAs(true)} disabled={loading}>
        {loading ? <ProgressIndicator color="inherit" size={20} /> : "Confirm & Settle"}
      </Button>
    </>
  );
    ...
    >
      <Box sx={{ display: "flex", flexDirection: "column", gap: 2, pt: 1 }}>
        <Alert severity="info">
          ...
          {invoice.due_date && ` | Due: ${formatDateToDDMMYYYY(invoice.due_date)}`}
        </Alert>

        <TextField fullWidth label="Amount (UGX) *" type="number"
        ...
        <FormControl fullWidth>
          ...
          <Select value={form.values.method} label="Payment Method *"
        ...
        <TextField fullWidth label="Reference Number *" value={form.values.reference_number}
          onChange={e => form.setFieldValue("reference_number", e.target.value)}
          error={Boolean(form.errors.reference_number)} helperText={form.errors.reference_number as string} />
        ...
        <Alert severity="warning" sx={{ fontSize: 12 }}>
          <strong>Confirm & Settle</strong> applies the payment immediately. If the invoice becomes
          fully paid, investor profit settlement runs automatically.
        </Alert>
      </Box>
    </ModalDialog>
  );
...
const BuyerInvoices: FC = () => {
  ...
  const tableActions: IDropdownAction[] = [
    ...
    },
    {
      ...
      condition: (inv: IBuyerInvoice) => ["issued", "partial"].includes(inv.status),
    },
  ];
    ...
    },
    {
      ...
      Cell: ({ row }: any) => (
        <Chip
          ...
          color={BUYER_INVOICE_STATUS_COLORS[row.original.status]}
          size="small"
        />
      ),
    },
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2, flexWrap: "wrap", gap: 1 }}>
        ...
        <FormControl size="small" sx={{ minWidth: 160 }}>
          ...
          >
            ...
            {["issued", "partial", "paid", "overdue", "cancelled"].map(s => (
