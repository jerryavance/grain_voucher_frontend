...
import {
  ...
  IBuyerCreditStatus, IBuyerOrderList, IBuyerInvoice,
...
const AddContactDialog: FC<{
  ...
  onClose: () => void;
  callBack: () => void;
  initialValues?: IBuyerContactPreference | null;
}> = ({ buyerId, open, onClose, callBack, initialValues }) => {
  const [loading, setLoading] = useState(false);
  const form = useFormik({
    initialValues: {
      buyer: buyerId,
      role: initialValues?.role || "procurement",
    ...
    onSubmit: async (values) => {
      ...
      try {
        if (initialValues) {
          await SourcingService.updateBuyerContact(initialValues.id, values);
          toast.success("Contact updated");
        } else {
          await SourcingService.createBuyerContact(values);
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
        ...
        <Grid container spacing={2} sx={{ pt: 1 }}>
          <Grid item xs={12} md={6}>
            <FormControl fullWidth>
              ...
              <Select value={form.values.role} label="Role"
                onChange={e => form.setFieldValue("role", e.target.value)}>
                {["procurement", "finance", "operations", "management", "other"].map(r => (
          ...
          <Grid item xs={12} md={6}>
            <TextField fullWidth label="Full Name *" value={form.values.name}
              onChange={e => form.setFieldValue("name", e.target.value)}
              error={Boolean(form.errors.name)} helperText={form.errors.name as string} />
          </Grid>
          <Grid item xs={12} md={6}>
            <TextField fullWidth label="Phone *" value={form.values.phone}
              onChange={e => form.setFieldValue("phone", e.target.value)}
              error={Boolean(form.errors.phone)} helperText={form.errors.phone as string} />
          </Grid>
          <Grid item xs={12} md={6}>
            <TextField fullWidth label="Email" value={form.values.email}
              onChange={e => form.setFieldValue("email", e.target.value)} />
          </Grid>
          <Grid item xs={12}>
            <FormControl fullWidth>
              <InputLabel>Primary Contact?</InputLabel>
              <Select value={form.values.is_primary ? "yes" : "no"} label="Primary Contact?"
                onChange={e => form.setFieldValue("is_primary", e.target.value === "yes")}>
                <MenuItem value="yes">Yes — primary contact</MenuItem>
                <MenuItem value="no">No — additional contact</MenuItem>
              </Select>
            </FormControl>
          </Grid>
        </Grid>
      ...
      <DialogActions>
        ...
        <Button variant="contained" onClick={() => form.handleSubmit()} disabled={loading}>
          {loading ? <ProgressIndicator color="inherit" size={20} /> : initialValues ? "Save" : "Add Contact"}
        </Button>
      </DialogActions>
    </Dialog>
  );
...
const BuyerDetails: FC = () => {
  ...
  const fetchAll = async () => {
    ...
    try {
      const [b, d, c, o, inv, cont] = await Promise.all([
        ...
        SourcingService.getBuyerOrders_byProfile(id!).catch(() => ({ results: [] })),
        SourcingService.getBuyerInvoices_byProfile(id!).catch(() => ({ results: [] })),
        SourcingService.getBuyerContacts({ buyer: id }).catch(() => ({ results: [] })),
      ]);
      setBuyer(b);
      setDashboard(d);
      setCredit(c);
      setOrders((o as any).results || []);
      setInvoices((inv as any).results || []);
  ...
  const handleVerify = async () => {
    ...
    try {
      await SourcingService.verifyBuyer(id!);
  ...
  const handleToggleActive = async () => {
    ...
    try {
      buyer.is_active ? await SourcingService.deactivateBuyer(id!) : await SourcingService.reactivateBuyer(id!);
  ...
  const handleDeleteContact = async (contactId: string) => {
    ...
    try {
      await SourcingService.deleteBuyerContact(contactId);
  ...
  const creditUsedPct = credit && credit.credit_limit > 0
    ? Math.min(100, ((credit.outstanding_balance / credit.credit_limit) * 100))
    : 0;
  ...
  return (
    <Box pt={2} pb={6}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 3, flexWrap: "wrap" }}>
        <Button startIcon={<ArrowBackIcon />} onClick={() => navigate("/admin/sourcing/buyers")} sx={{ mr: 1 }}>
          Buyers
        </Button>
        <Avatar sx={{ width: 44, height: 44, bgcolor: "primary.main", fontWeight: 700 }}>
          {buyer.business_name.charAt(0).toUpperCase()}
        </Avatar>
        <Box sx={{ flex: 1 }}>
          ...
          <Typography variant="body2" color="text.secondary">{buyer.contact_name} · {buyer.phone}</Typography>
        </Box>
        <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
          {!buyer.is_verified && (
            <Button variant="outlined" color="success" size="small" startIcon={<VerifiedIcon />} onClick={handleVerify}>
              Verify
            </Button>
          )}
          <Button variant="outlined" color={buyer.is_active ? "error" : "success"} size="small"
            onClick={handleToggleActive}>
            {buyer.is_active ? "Deactivate" : "Reactivate"}
          </Button>
          <Button variant="contained" size="small" startIcon={<AddIcon />}
            onClick={() => navigate(`/admin/sourcing/buyer-orders?buyer=${id}`)}>
            New Order
          </Button>
        </Box>
      </Box>
          ...
          <CardContent sx={{ p: 2, "&:last-child": { pb: 2 } }}>
            <Box sx={{ display: "flex", justifyContent: "space-between", mb: 0.5 }}>
              <Typography variant="body2" sx={{ fontWeight: 600 }}>
                Credit Utilisation {creditUsedPct > 80 && <WarningIcon sx={{ fontSize: 14, color: "error.main", ml: 0.5, verticalAlign: "middle" }} />}
              </Typography>
              <Typography variant="body2">
                {formatCurrency(credit.outstanding_balance)} / {formatCurrency(credit.credit_limit)}
                <Span sx={{ ml: 1, color: "success.main" }}>({formatCurrency(credit.credit_available)} available)</Span>
              </Typography>
            </Box>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            <Card variant="outlined"><CardContent>
              ...
              <Divider sx={{ my: 2 }} />
              <InfoRow label="Default Credit Terms" value={buyer.default_credit_terms_days === 0 ? "Cash on Delivery" : `Net ${buyer.default_credit_terms_days} days`} />
              <InfoRow label="Credit Limit" value={buyer.credit_limit > 0 ? formatCurrency(buyer.credit_limit) : "No limit set"} />
              <InfoRow label="Outstanding Balance" value={
                <Span sx={{ fontWeight: 700, color: buyer.outstanding_balance > 0 ? "error.main" : "success.main" }}>
                  {formatCurrency(buyer.outstanding_balance)}
                </Span>
              } />
              <InfoRow label="Credit Available" value={
                <Span sx={{ fontWeight: 700, color: "success.main" }}>
                  {buyer.credit_limit > 0 ? formatCurrency(buyer.credit_available) : "Unlimited"}
                </Span>
              } />
            </CardContent></Card>

            <Card variant="outlined" sx={{ mt: 2 }}><CardContent>
              <Typography variant="subtitle1" sx={{ fontWeight: 700, mb: 2 }}>Verification</Typography>
              <InfoRow label="Status" value={
                <Chip label={buyer.is_verified ? "Verified" : "Unverified"} color={buyer.is_verified ? "success" : "warning"} size="small" />
              } />
              <InfoRow label="Active" value={
                <Chip label={buyer.is_active ? "Active" : "Inactive"} color={buyer.is_active ? "success" : "error"} size="small" />
              } />
              {buyer.verified_at && <InfoRow label="Verified At" value={formatDateToDDMMYYYY(buyer.verified_at)} />}
              <InfoRow label="Registered On" value={formatDateToDDMMYYYY(buyer.created_at)} />
            </CardContent></Card>
          </Grid>
        </Grid>
      </TabPanel>
          ...
          <Table size="small">
            <TableHead>
              <TableRow sx={{ bgcolor: "action.hover" }}>
                {["Order #", "Revenue", "Gross Profit", "Status", "Invoice", "Date"].map(h => (
            ...
            <TableBody>
              {orders.length === 0 ? (
                ...
                <TableRow key={o.id} hover sx={{ cursor: "pointer" }}
                  ...
                  <TableCell sx={{ color: o.gross_profit >= 0 ? "success.main" : "error.main", fontWeight: 600 }}>
                    {formatCurrency(o.gross_profit)}
                  </TableCell>
                  <TableCell><Chip label={o.status.toUpperCase()} color={BUYER_ORDER_STATUS_COLORS[o.status]} size="small" /></TableCell>
                  <TableCell>
                    {o.invoice_status
                      ? <Chip label={o.invoice_status.toUpperCase()} color={BUYER_INVOICE_STATUS_COLORS[o.invoice_status]} size="small" />
                      : <Span sx={{ color: "text.secondary", fontSize: 12 }}>—</Span>}
                  </TableCell>
      ...
      <TabPanel value={tab} index={2}>
        <TableContainer component={Card} variant="outlined">
          <Table size="small">
            <TableHead>
              <TableRow sx={{ bgcolor: "action.hover" }}>
                {["Invoice #", "Amount Due", "Paid", "Balance", "Terms", "Due Date", "Status"].map(h => (
                  <TableCell key={h} sx={{ fontWeight: 700 }}>{h}</TableCell>
                ))}
              </TableRow>
            </TableHead>
            <TableBody>
              {invoices.length === 0 ? (
                <TableRow><TableCell colSpan={7} align="center" sx={{ py: 4, color: "text.secondary" }}>No invoices yet</TableCell></TableRow>
              ) : invoices.map(inv => (
                <TableRow key={inv.id} hover>
                  ...
                  <TableCell sx={{ fontWeight: 700, color: inv.balance_due > 0 ? "error.main" : "success.main" }}>
                    {formatCurrency(inv.balance_due)}
                  </TableCell>
                  <TableCell>{inv.payment_terms_days === 0 ? "On Delivery" : `Net ${inv.payment_terms_days}`}</TableCell>
                  <TableCell sx={{ fontSize: 12, color: inv.is_overdue ? "error.main" : "inherit" }}>
                    {inv.due_date ? formatDateToDDMMYYYY(inv.due_date) : "—"}{inv.is_overdue && " ⚠"}
                  </TableCell>
                  <TableCell><Chip label={inv.status.toUpperCase()} color={BUYER_INVOICE_STATUS_COLORS[inv.status]} size="small" /></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
      ...
      <TabPanel value={tab} index={3}>
        <Box sx={{ display: "flex", justifyContent: "flex-end", mb: 2 }}>
          <Button variant="contained" startIcon={<AddIcon />} onClick={() => { setEditContact(null); setShowAddContact(true); }}>
            Add Contact
          </Button>
        </Box>
        {contacts.length === 0 ? (
          <Alert severity="info">No additional contacts. The primary contact is: <strong>{buyer.contact_name}</strong> ({buyer.phone})</Alert>
        ) : (
                  ...
                  <CardContent>
                    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", mb: 1 }}>
                      <Box sx={{ display: "flex", gap: 1 }}>
                        ...
                        {c.is_primary && <Tooltip title="Primary contact"><StarIcon sx={{ color: "warning.main", fontSize: 18 }} /></Tooltip>}
                      </Box>
                      <Box>
                        <IconButton size="small" onClick={() => { setEditContact(c); setShowAddContact(true); }}>
      ...
      <AddContactDialog
        ...
        callBack={() => SourcingService.getBuyerContacts({ buyer: id }).then(r => setContacts((r as any).results || []))}
