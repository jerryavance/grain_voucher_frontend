...
//  * BuyerInvoicePDF.tsx
//  *
//  * Generates a buyer invoice via a print-window (same approach as SupplierInvoicePDF).
//  *
//  * Now consumes the full cost-breakdown fields returned by the updated backend:
//  *   - order_lines   → per-lot grain sale lines (qty, price, revenue, COGS)
//  *   - sale_expenses → itemised selling expenses
//  *   - subtotal / total_cogs / total_selling_expenses / gross_profit / gross_margin_pct
//  *   - buyer_detail  → full buyer contact block for invoice header
//  *
//  * Company colour: #2371B9
//  */
...
// import { Button, CircularProgress, IconButton, Tooltip } from "@mui/material";
// import PictureAsPdfIcon from "@mui/icons-material/PictureAsPdf";
// import { IBuyerInvoice, IBuyerInvoiceOrderLine, IBuyerInvoiceSaleExpense } from "./Sourcing.interface";
...
//         <td style="padding:8px 10px;border-bottom:1px solid #f0f4fa;font-weight:600;color:#1565c0;">${l.lot_number}</td>
//         <td style="padding:8px 10px;border-bottom:1px solid #f0f4fa;">${l.grain_type}</td>
//         <td style="padding:8px 10px;border-bottom:1px solid #f0f4fa;font-family:monospace;font-size:11px;">${l.source_order_number || "—"}</td>
//         <td style="padding:8px 10px;border-bottom:1px solid #f0f4fa;text-align:right;">${fmtNum(l.quantity_kg)} kg</td>
//         <td style="padding:8px 10px;border-bottom:1px solid #f0f4fa;text-align:right;">${ugx(l.sale_price_per_kg)}</td>
//         <td style="padding:8px 10px;border-bottom:1px solid #f0f4fa;text-align:right;font-weight:700;">${ugx(l.line_total)}</td>
//         <td style="padding:8px 10px;border-bottom:1px solid #f0f4fa;text-align:right;color:#888;">${ugx(l.cogs_per_kg)}</td>
//         <td style="padding:8px 10px;border-bottom:1px solid #f0f4fa;text-align:right;color:#d97706;">${ugx(l.cogs_total)}</td>
//         <td style="padding:8px 10px;border-bottom:1px solid #f0f4fa;text-align:right;font-weight:700;color:${Number(l.line_gross_profit) >= 0 ? "#15803d" : "#dc2626"};">${ugx(l.line_gross_profit)}</td>
//       </tr>`).join("")
//     : `<tr><td colspan="9" style="padding:12px 10px;text-align:center;color:#999;">No line items</td></tr>`;

//   // ── Selling expenses HTML ─────────────────────────────────────────────────
//   const expenses: IBuyerInvoiceSaleExpense[] = invoice.sale_expenses || [];
//   const expensesHtml = expenses.length > 0
//     ? expenses.map(e => `
//       <tr>
//         <td style="padding:7px 10px;border-bottom:1px solid #f0f4fa;">${e.category_display}</td>
//         <td style="padding:7px 10px;border-bottom:1px solid #f0f4fa;">${e.description}</td>
//         <td style="padding:7px 10px;border-bottom:1px solid #f0f4fa;font-family:monospace;font-size:11px;">${e.receipt_reference || "—"}</td>
//         <td style="padding:7px 10px;border-bottom:1px solid #f0f4fa;text-align:right;font-weight:600;color:#d97706;">${ugx(e.amount)}</td>
//       </tr>`).join("")
//     : `<tr><td colspan="4" style="padding:10px;text-align:center;color:#999;">No selling expenses</td></tr>`;
...
//   .summary-row .s-label { color:#555; }
//   .summary-row .s-value { font-weight:700; }
//   .summary-row.cogs   .s-value { color:#d97706; }
//   .summary-row.profit { background:#f6f9fd; }
//   .summary-row.profit .s-value { color:${grossProfit >= 0 ? "#15803d" : "#dc2626"};font-size:14px;font-weight:800; }
//   .summary-row.balance { background:#2371B9;color:#fff;padding:12px 12px; }
//   .summary-row.balance .s-label { color:rgba(255,255,255,0.7);font-size:11px; }
//   .summary-row.balance .s-value { color:#fff;font-size:16px; }
//   .margin-chip { display:inline-block;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700;background:${grossMarginPct >= 0 ? "#dcfce7" : "#fee2e2"};color:${grossMarginPct >= 0 ? "#15803d" : "#dc2626"}; }
...
//       <div class="meta-value"><span class="margin-chip">${grossMarginPct.toFixed(2)}%</span></div>
//     </div>
//     <div class="meta-cell dark">
//       <div class="meta-label">Status</div>
//       <div class="meta-value"><span class="status-badge">${ss.label}</span></div>
...
//   <!-- Grain Sale Lines -->
//   <div class="section-heading">Grain Sale Lines</div>
//   <table class="data-table">
//     <thead>
//       <tr>
//         <th>Lot #</th>
...
//         <th class="r">Sale Price/kg</th>
//         <th class="r">Revenue</th>
//         <th class="r">COGS/kg</th>
//         <th class="r">COGS</th>
//         <th class="r">Line Profit</th>
//       </tr>
//     </thead>
//     <tbody>
//       ${linesHtml}
...
//     <tfoot>
//       <tr>
//         <td colspan="5" style="text-align:right;font-size:11px;color:#5c8abf;">Subtotal (Revenue)</td>
//         <td style="text-align:right;font-size:13px;font-weight:800;">${ugx(subtotal)}</td>
//         <td style="text-align:right;color:#d97706;">COGS</td>
//         <td style="text-align:right;font-size:13px;font-weight:700;color:#d97706;">${ugx(totalCogs)}</td>
//         <td></td>
//       </tr>
//     </tfoot>
//   </table>
...
//   <!-- Selling Expenses -->
//   <div class="section-heading">Selling Expenses</div>
//   <table class="data-table">
//     <thead>
//       <tr>
//         <th>Category</th>
...
//     <tfoot>
//       <tr>
//         <td colspan="3" style="text-align:right;font-size:11px;color:#5c8abf;">Total Selling Expenses</td>
//         <td style="text-align:right;font-size:13px;font-weight:700;color:#d97706;">${ugx(totalExpenses)}</td>
//       </tr>
//     </tfoot>` : ""}
//   </table>
...
//     <!-- P&L + Payment Summary -->
//     <div>
//       <div class="section-heading" style="margin-top:0;">P&amp;L &amp; Payment Summary</div>
//       <div class="summary-box">
//         <div class="summary-row">
//           <span class="s-label">Revenue (Subtotal)</span>
//           <span class="s-value">${ugx(subtotal)}</span>
//         </div>
//         <div class="summary-row cogs">
//           <span class="s-label">Cost of Goods Sold</span>
//           <span class="s-value">(${ugx(totalCogs)})</span>
//         </div>
//         ${totalExpenses > 0 ? `
//         <div class="summary-row cogs">
//           <span class="s-label">Selling Expenses</span>
//           <span class="s-value">(${ugx(totalExpenses)})</span>
//         </div>` : ""}
//         <div class="summary-row profit">
//           <span class="s-label" style="font-weight:700;">Gross Profit</span>
//           <span class="s-value">${ugx(grossProfit)}</span>
//         </div>
//         <div class="summary-row">
//           <span class="s-label">Invoice Amount (Receivable)</span>
//           <span class="s-value">${ugx(amountDue)}</span>
//         </div>
//         ${amountPaid > 0 ? `
//         <div class="summary-row" style="color:#15803d;">
